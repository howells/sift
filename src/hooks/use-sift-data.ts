...
export function useSiftData(): SiftState {
  ...
  function removeTodoFromList(todoId: string): void {
    if (view === "backlog") {
      ...
      setAllTodos((prev) => {
        ...
        if (newTodos.length < 5 && !loading.active && !autoRefreshQueued.current) {
  ...
  function moveDown() {
    setSelectedIndex((i) => Math.min(Math.max(0, currentList.length - 1), i + 1));
  }
  ...
  function markDone() {
    ...
    void Promise.resolve(runTodoAction("done", todo, siftData)).then((result) => {
      if (!result.success) {
        return;
      }
  ...
  function starTodo() {
    ...
    if (!todo.isStarred) {
      void Promise.resolve(runTodoAction("star", todo, siftData)).then((result) => {
        if (result.success) {
  ...
  function openTodo() {
    ...
    void Promise.resolve(runTodoAction("open", todo, siftData)).then((result) => {
      if (result.success && result.error) {
        void open(result.error);
      }
    });
  }
