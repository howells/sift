...
export function migrateSchema(database: Database.Database): Database.Database {
  ...
  if (currentVersion < 2) {
    ...
    if (!columns.some((column) => column.name === "action_type")) {
      database.exec("ALTER TABLE analyzed_emails ADD COLUMN action_type TEXT");
    }
  }
...
const stmts = {
  get: db.prepare(`
    SELECT email_id, account, thread_id, analyzed_at, email_hash, is_todo, todo_json, action_type
    FROM analyzed_emails
    WHERE email_id = ?
  `),
  insert: db.prepare(`
    INSERT OR REPLACE INTO analyzed_emails
      (email_id, account, thread_id, analyzed_at, email_hash, is_todo, todo_json, action_type)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  `),
 ...
 */
export function getCachedAnalysis(emailId: string, currentHash: string): CachedAnalysis | null {
