...
describe(validateId, () => {
  it("accepts valid Gmail thread IDs", () => {
    expect(validateId("19d42c92f393e035")).toStrictEqual({ valid: true });
    expect(validateId("abc123")).toStrictEqual({ valid: true });
    expect(validateId("reminder-abc-123")).toStrictEqual({ valid: true });
  });
  ...
  it("rejects control characters", () => {
    const result = validateId("abc\u0000def");
    expect(result.valid).toBeFalsy();
  ...
  it("rejects path traversal", () => {
    ...
    expect(result.valid).toBeFalsy();
    expect("error" in result && result.error).toContain("path traversal");
  });

  it("rejects query params", () => {
    const result = validateId("abc?foo=bar");
    expect(result.valid).toBeFalsy();
    expect("error" in result && result.error).toContain("query params");
  });
  ...
  it("rejects spaces and special chars", () => {
    expect(validateId("abc def").valid).toBeFalsy();
    expect(validateId("abc@def").valid).toBeFalsy();
    expect(validateId("abc;def").valid).toBeFalsy();
  });
});
...
describe(validateGroup, () => {
  ...
  it("accepts valid groups", () => {
    expect(validateGroup("personal", validGroups)).toStrictEqual({ valid: true });
    expect(validateGroup("siteinspire", validGroups)).toStrictEqual({ valid: true });
  });
  ...
  it("rejects unknown groups", () => {
    ...
    expect(error).toContain("personal");
  });
...
describe(validateFields, () => {
  const validFields = ["id", "summary", "urgency", "person", "deadline", "account"];

  it("accepts valid field lists", () => {
    expect(validateFields("id,summary,urgency", validFields)).toStrictEqual({
      valid: true,
    });
  });

  it("accepts single fields", () => {
    expect(validateFields("id", validFields)).toStrictEqual({ valid: true });
  });

  it("rejects unknown fields", () => {
    const result = validateFields("id,foobar,summary", validFields);
    expect(result.valid).toBeFalsy();
    expect("error" in result && result.error).toContain("foobar");
  });
