...
export async function getAuthenticatedClient(
	...
	const existingToken = (await loadToken(accountName)) as {
		expiry_date?: number;
	} | null;
	if (existingToken) {
		...
		if (isExpiring) {
			try {
				...
				console.log(
					`Token refresh failed for ${accountName}, re-authenticating...`,
				);
	...
	console.log(
		"Please sign in with the correct Google account in the browser.\n",
	);
