...
export function getIntegerFlag(flags: Map<string, string | true>, key: string): number | null {
...
export function writeAgentOutput(data: object, flags?: Map<string, string | true>): void {
  ...
  if (format === "ndjson" && Array.isArray(sanitized.items)) {
    for (const item of sanitized.items) {
      process.stdout.write(`${JSON.stringify({ data: item, type: "item" })}\n`);
    }
    if (sanitized.page) {
      process.stdout.write(`${JSON.stringify({ data: sanitized.page, type: "page" })}\n`);
    }
    return;
  }
