...
function toMcpToolName(commandKey: string): string {
	const sanitized = commandKey.replace(/[.:]/g, "_").replace(/-/g, "_");
...
function buildFlagSchema(flag: CommandFlagSchema): ZodTypeAny {
	...
	switch (flag.type) {
		...
		default:
			schema = flag.enum
				? z.enum(flag.enum as [string, ...string[]])
				: z.string();
			break;
	}
...
function buildToolDescription(schema: CommandSchema): string {
	...
	if (schema.output.supportsFields) {
		lines.push(
			"",
			"Use `fields` (comma-separated) to keep the response narrow.",
		);
	}
...
function buildAnnotations(
	schema: CommandSchema,
): SiftToolDefinition["annotations"] {
