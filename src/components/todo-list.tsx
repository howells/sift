...
function buildFlatList(overdue: Todo[], thisWeek: Todo[], whenYouCan: Todo[]): ListItem[] {
  const flatList: ListItem[] = [];
  let globalIndex = 0;

  const addSection = (title: string, icon: string, items: Todo[], color: string) => {
    if (items.length === 0) {
      return;
    }
    flatList.push({ color, count: items.length, icon, title, type: "header" });
    for (const todo of items) {
      flatList.push({ globalIndex, todo, type: "todo" });
      globalIndex++;
    }
  };
...
function findFlatListWindow(
  ...
  for (let i = 0; i < flatList.length; i++) {
    ...
    if (item.type === "todo") {
      if (todoCount === windowStart) {
        const previousItem = i > 0 ? flatList[i - 1] : undefined;
        flatStart = previousItem?.type === "header" ? i - 1 : i;
        break;
      }
      todoCount++;
    }
  ...
  return { flatEnd, flatStart };
}

export function TodoList({ selectedIndex, showAccount = true, todos }: TodoListProps) {
  ...
  const { flatStart, flatEnd } = findFlatListWindow(flatList, windowStart, windowEnd);
  ...
  return (
    <Box flexDirection="column">
      ...
      {windowEnd < todos.length && <Text dimColor> ↓ {todos.length - windowEnd} more</Text>}
    </Box>
  );
}
